...
VMINT job(VMWCHAR *file_path, VMINT wlen)
{
    ...
    VMWCHAR ext_check1[6];
    VMWCHAR ext_check2[5];
    VMWCHAR ext_check3[7];

    vm_ascii_to_ucs2(ext_check1, sizeof(ext_check1), ".html");
    vm_ascii_to_ucs2(ext_check2, sizeof(ext_check2), ".htm");
    vm_ascii_to_ucs2(ext_check3, sizeof(ext_check3), ".xhtml");

    if (len >= 6 && vm_wstrcmp(file_path + len - 5, ext_check1) == 0)
    {
        cut_len = len - 5;
    }
    else if (len >= 5 && vm_wstrcmp(file_path + len - 4, ext_check2) == 0)
    {
        cut_len = len - 4;
    }
    else if (len >= 7 && vm_wstrcmp(file_path + len - 6, ext_check3) == 0)
    {
        cut_len = len - 6;
    }
